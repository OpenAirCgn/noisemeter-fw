...
//  LED_Set(max-min > 0x1000);
  ...
  q15_t dba[numSamples];
  arm_biquad_cascade_df1_q15(&dBA_filter, src, dba, numSamples);
  
  //biquads may offset zero so compensate mean to zero. Important for RMS
  q15_t mean;
  arm_mean_q15 (dba, numSamples, &mean);  
  dBA_mean = (DBA_MEAN_SMOOTH * dBA_mean + mean) / (DBA_MEAN_SMOOTH+1);
  arm_offset_q15 ( dba, -dBA_mean, dba, numSamples);
